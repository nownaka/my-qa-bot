...
> [!WARNING]
> タスク実行は `macOS` のみ対応しています。（2024/08/31 現在）

### 1. ターミナルから Azure にログインします。
...
#### 2-1. ショートカットキー：`⌘` + `shift` + `p` を入力し、「タスク：タスクの実行」を選択する。
...
#### 3-1. ショートカットキー：`⌘` + `shift` + `p` を入力し、「タスク：タスクの実行」を選択する。

![image](https://github.com/user-attachments/assets/d3196bf5-13c5-4317-9f1a-594d6b429126)

#### 3-2. 「Azure 上にコンポーネントを作成する(macOS)」を選択する。
...
#### 4-1. ショートカットキー：`⌘` + `shift` + `p` を入力し、「タスク：タスクの実行」を選択する。

![image](https://github.com/user-attachments/assets/d3196bf5-13c5-4317-9f1a-594d6b429126)

#### 4-2. 「アプリをデプロイする(macOS)」を選択する。
